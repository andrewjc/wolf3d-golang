...
func (c *RenderView) renderWalls(m *image.RGBA) {
    ...
    for x := 0; x < c.renderWidth; x++ {
        ...
        for y := drawStart; y < drawEnd+1; y++ {
            ...
            percentage = applyDistanceFalloff(percentage, perpWallDist, c.parent.game.currentTick)
            if percentage < 1e-6 {
                percentage = 1e-6
            }
        ...
        for y := drawEnd + 1; y < c.renderHeight; y++ {
            ...
            percentage = applyDistanceFalloff(percentage, perpFloorDist, c.parent.game.currentTick)
            if percentage < 1e-6 {
                percentage = 1e-6
            }
...
func applyDistanceFalloff(percentage float64, dist float64, tick int64) float64 {
    if dist > 0 {
        return percentage * math.Pow(percentage, 1.2)
    } else {
        return percentage
    }
}
