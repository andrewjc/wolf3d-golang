...
class Runner(object):
    ...
    def __init__(self, net, env, num_envs, n_stack, rollout_size=5, num_updates=2500000, max_grad_norm=0.5,
                 value_coeff=0.5, entropy_coeff=0.02, tensorboard_log=True, log_path="./log", is_cuda=True, seed=42):
        super().__init__()
        ...
        self.storage = RolloutStorage(self.rollout_size, self.num_envs, self.env.observation_space.shape,
    ...
    def train(self):
        ...
        for num_update in range(self.num_updates):
            ...
            if self.writer is not None and len(self.storage.episode_rewards) > 1:
                self.writer.add_histogram("episode_rewards", torch.tensor(self.storage.episode_rewards), num_update)
