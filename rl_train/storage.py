...
class RolloutStorage(object):
    ...
    def reset_buffers(self):
        ...
        self.states = self._generate_buffer((self.rollout_size + 1, self.num_envs, *self.frame_shape))
    ...
    def obs2tensor(self, obs):
        ...
        tensor = torch.from_numpy(obs.astype(np.float32)) / 255.
    ...
    def a2c_loss(self, final_value, entropy, step):
        # calculate advantage
        ...
        if self.writer is not None:
            self.writer.add_scalar("a2c_loss", loss.item(), step)
            self.writer.add_scalar("policy_loss", policy_loss.item(), step)
            self.writer.add_scalar("value_loss", value_loss.item(), step)
            self.writer.add_histogram("advantage", advantage.detach(), step)
            self.writer.add_histogram("rewards", rewards.detach(), step)
            self.writer.add_histogram("action_prob", self.log_probs.detach(), step)
